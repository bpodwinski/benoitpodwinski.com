 ...
 */
THREE.BAS.Timeline.prototype.add = function (
  duration,
  transitions,
  positionOffset
) {
...
THREE.BAS.Timeline.prototype.processTransition = function (
  key,
  transition,
  start,
  duration
...
THREE.BAS.ShaderChunk["quaternion_slerp"] =
...
THREE.BAS.ModelBufferGeometry.prototype.computeCentroids = function () {
  ...
  for (var i = 0; i < this.faceCount; i++) {
    this.centroids[i] = THREE.BAS.Utils.computeCentroid(
      this.modelGeometry,
  ...
  if (localizeFaces === true) {
    for (i = 0; i < this.faceCount; i++) {
      ...
      var centroid = this.centroids
        ? this.centroids[i]
        : THREE.BAS.Utils.computeCentroid(this.modelGeometry, face);
 ...
 */
THREE.BAS.ModelBufferGeometry.prototype.setFaceData = function (
  attribute,
  faceIndex,
  data
) {
...
THREE.BAS.StandardAnimationMaterial = function (parameters) {
  ...
  THREE.BAS.BaseAnimationMaterial.call(
    this,
    parameters,
...
THREE.BAS.Timeline.register("rotate", {
  compiler: function (segment) {
    ...
    return [
      ...
      THREE.BAS.TimelineChunks.vec4(
        "cRotationFrom" + segment.key,
        fromAxisAngle,
        8
      ),
      THREE.BAS.TimelineChunks.vec4(
        "cRotationTo" + segment.key,
        toAxisAngle,
        8
      ),
      origin &&
        THREE.BAS.TimelineChunks.vec3("cOrigin" + segment.key, origin, 2),
...
THREE.BAS.Timeline.register("scale", {
  compiler: function (segment) {
    ...
    return [
      THREE.BAS.TimelineChunks.delayDuration(segment),
      THREE.BAS.TimelineChunks.vec3(
        "cScaleFrom" + segment.key,
        segment.transition.from,
        2
      ),
      THREE.BAS.TimelineChunks.vec3(
        "cScaleTo" + segment.key,
 ...
 * @constructor
 */
THREE.BAS.TimelineSegment = function (
  key,
  start,
  duration,
  transition,
