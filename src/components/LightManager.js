...
export class LightManager {
    ...
    constructor(scene) {
        ...
        this.log = this.debugMode ? console.log.bind(console, "[LightManager]") : () => {};
    ...
    validateConfig(config) {
        ...
        if (!Array.isArray(config.position) || config.position.length !== 3) {
            throw new Error("Light position must be an array of three numbers [x, y, z].");
        }
    }
    ...
    configureShadows(light, shadowConfig) {
        if (!shadowConfig || !(light instanceof THREE.DirectionalLight || light instanceof THREE.PointLight)) {
            return;
        }
